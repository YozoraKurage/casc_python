...
APIリファレンスは2系統あります。用途で使い分けてください。

| | [手書き版 `api/`](api/README.md) | [公式クローン版 `api-reference/`](api-reference/README.md) |
|---|---|---|
| 内容 | モジュール単位で要点を再構成・日本語補足あり | 公式サイトの**1ページ=1md**を全209ページ忠実に写したもの |
| 向き | 概念理解・読み物として | 網羅的に引く・最新シグネチャの確認・差分比較 |
| 生HTML | — | [`api-reference/raw-html/`](api-reference/raw-html/) に全213ページのダンプを保持 |

**手書き版**は `csc` の全サブモジュールを以下の表から辿れます。
